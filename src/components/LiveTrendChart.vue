...
const uid = `live-trend-${Math.random().toString(36).slice(2, 9)}`
const hoveredIndex = ref(null)
const reveal = ref(0)
const rootRef = ref(null)

const pad = { top: 18, right: 22, bottom: 46, left: 42 }
const width = 520
...
<template>
  ...
  <div
    v-else
    ref="rootRef"
    class="live-trend-chart"
    :class="{ 'live-trend-chart--compact': compact }"
    @mousemove="onMove"
    @mouseleave="onLeave"
    @touchmove.prevent="onMove($event.touches[0])"
    @touchend="onLeave"
  >
    <div v-if="activePoint" class="live-trend-chart__tooltip" :style="tooltipStyle">
    ...
    >
      ...
      <g class="live-trend-chart__grid">
        <line
      ...
      <template v-if="type === 'line'">
        <path
          class="live-trend-chart__area"
          :d="areaPath"
          :fill="`url(#${uid}-fill)`"
          :style="{ opacity: reveal }"
        />
        <path
          class="live-trend-chart__line"
          :d="linePath"
          :stroke="color"
          fill="none"
          :style="{
        ...
        <line
          v-if="activePoint"
          class="live-trend-chart__crosshair"
          :x1="activePoint.x"
          :x2="activePoint.x"
          :y1="pad.top"
          :y2="pad.top + plotH"
      ...
      <g class="live-trend-chart__labels">
        <text
          ...
          "
          :class="{ 'live-trend-chart__labels--active': hoveredIndex === point.index }"
        >
          {{ point.label }}
        </text>
      </g>
    ...
    <p v-if="!compact" class="live-trend-chart__hint">Hover or drag across the chart for live values</p>
  </div>
</template>
